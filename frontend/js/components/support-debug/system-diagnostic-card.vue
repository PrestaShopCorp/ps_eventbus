  ...
  <section class="system-diagnostic">
    <div class="system-diagnostic__header">
      ...
      <PuikButton
        ...
        :disabled="supportDebugStore.rows.length === 0"
        @click="supportDebugStore.copyDiagnostic()"
      >
    ...
    <template v-else>
      ...
      <div v-if="supportDebugStore.rows.length > 0" class="system-diagnostic__list">
        <!-- `id` identifies the row for the v-for key only; binding it would
             land as a DOM id attribute. -->
        <DiagnosticRow
          v-for="row in supportDebugStore.rows"
          :key="row.id"
          :label="row.label"
          :hint="row.hint"
          :value="row.value"
