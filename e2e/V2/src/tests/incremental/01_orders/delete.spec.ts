...
const testTags = ['@incremental', '@orders-incremental', '@orders-incremental-delete'];
const testTitle = `${colorText('[INCREMENTAL]', ["bold", "cyan", "italic"])} ==> ORDER DELETE`;
// Test variables
...
mergedFixtures(testTitle, {tag: testTags}, async ({forceFullSync, bo_login, createCarrier, createAddress, createOrder}) => {
  ...
  await test.step('Delete created order', async () => {});
